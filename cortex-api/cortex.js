 ...
 */
class Cortex {
  constructor(user, socketUrl, profileName) {
    // create socket
  ...
  unSub(stream, authToken, sessionId) {
    let socket = this.socket;
    const SUB_REQUEST_ID = 6;
    let subRequest = {
      jsonrpc: "2.0",
  ...
  async querySessionInfo() {
    ...
    let ctResult = "";
    await this.controlDevice(headsetId)
      .then((result) => {
        ctResult = result;
      })
      .catch((err) => {
    ...
    let sessionId = "";
    await this.createSession(authToken, headsetId)
      .then((result) => {
        sessionId = result;
      })
      .catch((err) => {
  ...
  setupProfile(authToken, headsetId, profileName, status) {
    ...
    console.log("profile request", setupProfileRequest);
    let socket = this.socket;
    return new Promise(function (resolve, reject) {
  ...
  trainRequest(authToken, sessionId, action, status) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      socket.on("message", (data) => {
        ...
        if (status == "start") {
          try {
            if (JSON.parse(data)["sys"][1] == "MC_Failed") {
   ...
   *
   */
  async train(trainingActions, numberOfTrain, client) {
    console.log("SADKJASH JKDHAKJS HDKJASH KDJHASKJD H");
    ...
    for (let trainingAction of trainingActions) {
      for (let numTrain = 0; numTrain < numberOfTrain; numTrain++) {
        ...
        console.log("\r\n");
        const response = await self.trainRequest(
          self.authToken,
          self.sessionId,
          trainingAction,
          "start"
  ...
  async acceptTrain(profileName, trainingAction) {
    //accept 'neutral' result
    console.log(`ACCEPT "${trainingAction}"`);
    console.log("\r\n");
    await this.trainRequest(
      this.authToken,
      this.sessionId,
      trainingAction,
      "accept"
  ...
  async rejectTrain(trainingAction) {
    //reject 'neutral' result
    console.log(`REJECT "${trainingAction}"`);
    console.log("\r\n");
    await this.trainRequest(
      this.authToken,
      this.sessionId,
  ...
  unlive() {
    if (this.socket && this.socket.readyState === 1) {
      this.unSub(["com"], this.authToken, this.sessionId);
    }
  }
}
